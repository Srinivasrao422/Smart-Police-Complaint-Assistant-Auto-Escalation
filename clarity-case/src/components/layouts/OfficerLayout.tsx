...
import { clearSession, broadcastSessionChange } from "@/lib/session";
      ...
      <div className="p-3 border-t border-sidebar-border">
        <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-sidebar-accent">
          <div className="h-9 w-9 rounded-full gradient-teal flex items-center justify-center font-semibold text-sm text-secondary-foreground">
            {String(user?.name || "OF").split(" ").map((part: string) => part[0]).join("").slice(0, 2)}
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium truncate">{user?.name || "Officer"}</div>
            <div className="text-xs text-sidebar-foreground/60 truncate">
              {user?.department || "Operations"}
            </div>
          </div>
          <button onClick={logout} className="text-sidebar-foreground/60 hover:text-sidebar-foreground" aria-label="Logout">
  ...
  return (
    <div className="min-h-screen bg-transparent flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="border-b border-border bg-card/60 backdrop-blur sticky top-0 z-30">
          <div className="px-4 sm:px-6 py-3 flex items-center gap-3">
            ...
            <div className="hidden md:block">
              <h1 className="font-display text-lg font-bold leading-tight">Officer Dashboard</h1>
              <p className="text-xs text-muted-foreground">Assigned complaints and live status changes</p>
            </div>
        ...
        <main className="flex-1 p-4 sm:p-6 portal-stage">
          <Outlet />
        </main>
      </div>
      <ConsentNotice />
